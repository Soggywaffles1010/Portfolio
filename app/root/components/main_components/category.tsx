...
const Category: React.FC<CategoryProps> = ({ categories, onSelectCategory }) => {
  ...
  return (
    <div className="flex gap-4 md:p-4 p-2 translate-y-[150%] md:translate-y-[80%] overflow-x-auto scrollbar-hide">
      {categories.map((cat) => (
        <button
          key={cat}
          onClick={() => handleCategoryClick(cat)}
          className={`transition-transform duration-500 sm:py-1 sm:px-2 text-xs sm:text-sm xl:py-2 xl:px-4 xl:text-base rounded-sm ${
            activeCategory === cat
